...
CCScene* Playscene::scene()
{
	...
	//创建一个当前类型的层
	Playscene* player = Playscene::create();
	//将层作为子节点添加到场景
...
bool Playscene::init()
{
	...
	char str1[64] = { 0 };
	sprintf(str1,"misson%d.png",GameData::s_iCurLvevel);
	CCSprite* misson = CCSprite::create(str1);
	misson->setPosition(ccp(1234,750/4));
	misson->setScale(0.6f);
	this->addChild(misson,1);
	...
	Reback = CCMenuItemSprite::create(
		...
		OnTouchReback,
		this,
		menu_selector(Playscene::Button)); 
	...
	/*p_Map = GameMap::create("map1_1.tmx");
	...
	this->GameInitObject0();
	this->GameInitObject1();
	this->GameInitObject2();
	this->GameInitObject3();
	this->GameInitObject4();
	this->GameInitObject5();
	this->setTouchEnabled(true);
	return true;
}

void Playscene::Button(CCObject* pSender)
{
	...
	CCScene* ToIcon = LvChoiceLayer::scene();
	CCTransitionRotoZoom* ptran = CCTransitionRotoZoom::create(1.0f,ToIcon);

	CCDirector* director1 = CCDirector::sharedDirector();
	CCScene* ToPlayscene = Playscene::scene();
	CCTransitionFade* ptran1 = CCTransitionFade::create(1.0f,ToPlayscene);

	CCDirector* director2 = CCDirector::sharedDirector();
	CCScene* ToNextLv = Playscene::scene();
	CCTransitionFade* ptran2 = CCTransitionFade::create(1.0f,ToNextLv);
...
//初始化主角
void Playscene::GameInitObject0()
{
	//加载英雄
...
//初始化钥匙和门
void Playscene::GameInitObject1()
{
	//初始化钥匙
	//先判断是否存在英雄对象数据. 参数:对象组名字;对象名字
...
//初始化宝箱
void Playscene::GameInitObject2()
{
	char boxName[32] = {0};
	for (int i=0;i<3;++i)
	{
...
//初始化陷阱
void Playscene::GameInitObject3()
{
	//sizeof() 作用:求内存大小
...
//初始化怪物
void Playscene::GameInitObject4()
{
	//sizeof() 作用:求内存大小
...
//初始化传送门
void Playscene::GameInitObject5()
{
...
//检测碰撞
bool Playscene::playerCOllision(CCNode* obj)
{
	// 使用指针:首先判断是否有指向
	if (obj!=NULL)
	{
...
void Playscene::update(float dt)
{
	...
	//判断门碰撞检测
	if(playerCOllision(m_exit))
	{
		if(GameData::s_iCurLvevel <= 30)
		{
		if(m_key == NULL)
		{
	...
	{
		CCSprite* game_over = CCSprite::create("choicebg1.png");
		game_over->setPosition(ccp(1334/2,750/2));
	...
	//	if (m_Monster[i] != NULL)
	//	{
	//		if (playerCOllision(m_Monster[i]))
	//		{
	//			CCDirector::sharedDirector()->replaceScene(Playscene::scene());
	//	
	...
	for(int i = 0 ; i < len1 ; i++)
	{
		for(int j = 0 ; j < 2 ; j++)
		{
			...
			{
				if(playerCOllision(m_Transfer[i][j]))
				{
...
CCActionInterval* createRFAnimation(const char *frameName, int frameNum, float frameTime)
{
 ...
 void Playscene::registerWithTouchDispatcher(void)
{
